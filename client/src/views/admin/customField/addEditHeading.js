...
const AddEditHeading = (props) => {
  const { moduleId, filed: _filed, updateData, setUpdateData } = props;
  ...
  const fetchAddData = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || "Unable to save heading.");
    }
  };
  ...
  const editData = async () => {
    try {
      ...
      let response = await putApi(
        `api/custom-field/change-single-heading/${updateData?._id}`,
        editDataPayload
      );
      if (response?.status === 200) {
        handleClose();
      ...
      toast.error(error?.response?.data?.message || "Unable to update heading.");
    }
  };
      ...
      <Modal onClose={() => handleClose()} isOpen={props.isOpen} isCentered>
        ...
        <ModalContent maxWidth={"2xl"}>
          ...
          <ModalFooter>
            ...
            >
              {isLoding ? <Spinner /> : updateData && updateData?.heading ? "Update" : "Save"}
            </Button>
            <Button
              variant="outline"
              colorScheme="red"
