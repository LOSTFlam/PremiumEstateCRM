...
const AddEditUser = (props) => {
  const { onClose, isOpen, setAction, data, userAction, selectedId, fetchData, setUserAction } =
    props;
  const [isLoding, setIsLoding] = useState(false);
  const [show, setShow] = React.useState(false);
  const [roles, setRoles] = React.useState([]);
  const [roleModelOpen, setRoleModelOpen] = useState(false);
  const [assignToRoleData, _setAssignToRoleData] = useState([]);
              ...
              <Flex alignItems={"end"} justifyContent={"space-between"}>
                <Flex w={"100%"} flexDirection="column">
                  <CUIAutoComplete
                    ...
                    borderColor={errors?.roles && touched?.roles ? "red.300" : null}
                  />
                </Flex>
                <IconButton
                  mb={6}
