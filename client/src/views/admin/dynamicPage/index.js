...
const Index = () => {
  ...
  const [_selectedColumns, _setSelectedColumns] = useState([]);
        ...
        ?.map((field) => ({
          ...
          cell: (cell) => (
            <div className="selectOpt">
              <Text
                onClick={() => {
                  navigate(`/${title}/${cell?.row?.original?._id}`, {
                    state: { module: singaleData },
                  });
                }}
                me="10px"
        ...
        <Grid templateColumns="repeat(6, 1fr)" mb={3} gap={4}>
          <GridItem colSpan={6}>
            <CommonCheckTable
              ...
              tableData={data}
              tableCustomFields={
                moduleData?.fields?.filter((field) => field?.isTableField === true) || []
              }
              access={permission}
              // action={action}
              // setAction={setAction}
