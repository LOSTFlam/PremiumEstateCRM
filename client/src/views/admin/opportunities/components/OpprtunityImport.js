...
import { Flex, Select, Button, Grid, GridItem, Text } from "@chakra-ui/react";
...
function OpprtunityImport() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm: _resetForm }) => {
      const opportunityData = importedFileData?.map((item, _ind) => {
        const opportunity = {
  ...
  const { values, handleChange, handleSubmit, setFieldValue, resetForm } = formik;
