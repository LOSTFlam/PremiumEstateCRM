...
import { getSearchData, setGetTagValues } from "../../../redux/slices/advanceSearchSlice";
...
const Index = () => {
  ...
  const [tableColumns, _setTableColumns] = useState([]);
  const [columns, setColumns] = useState([]);
  const [_dataColumn, setDataColumn] = useState([]);
  ...
  const setStatusData = async (cell, e) => {
    try {
      setIsLoding(true);
      let response = await putApi(`api/lead/changeStatus/${cell?.original?._id}`, {
      ...
      const tempTableColumns = [
          ...
          cell: ({ row }) => (
            <div className="selectOpt">
              <Select
                ...
                style={{ fontSize: "14px" }}
              >
                <option value="active">{t?.("modules.lead.statusOptions.active")}</option>
                <option value="sold">{t?.("modules.lead.statusOptions.sold")}</option>
                <option value="pending">{t?.("modules.lead.statusOptions.pending")}</option>
              </Select>
            </div>
          ),
        },
        ...
        ...(leadData && leadData?.length > 0
          ? leadData[0]?.fields
              ?.filter(
                (field) =>
                  field?.isTableField === true && !field?.isView && field?.name !== "leadStatus"
              )
              ?.map((field) => ({
                Header: t?.(`fields.${field?.name}`) || field?.label,
                accessor: field?.name,
      ...
      <Grid templateColumns="repeat(6, 1fr)" mb={3} gap={4}>
        <GridItem colSpan={6}>
          <CommonCheckTable
              ...
              tableCustomFields={
                leadData?.[0]?.fields?.filter((field) => field?.isTableField === true) || []
              }
              access={permission}
              action={action}
              setAction={setAction}
