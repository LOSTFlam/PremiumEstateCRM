...
import { Link, useNavigate, useParams, useLocation as _useLocation } from "react-router-dom";
import { toast } from "react-toastify";
import { getApi, deleteApi } from "services/api";
import _PhoneCall from "../contact/components/phonCall";
...
const View = () => {
  ...
  const [
    ...
    taskAccess,
    meetingAccess,
  ] = HasAccess(["Leads", "Tasks", "Meetings", "Calls", "Emails", "Tasks", "Meetings"]);
  ...
  const tableColumns = [
    ...
    {
      Header: t("modules.lead.view.columns.lastCommunication"),
      accessor: "lastCommunicationDate",
      cell: (cell) => (
        <div className="selectOpt">
          <Text color={textColor} fontSize="sm" fontWeight="700">
            {cell?.row?.values.lastCommunicationDate
              ? moment(cell?.row?.values.lastCommunicationDate).format("DD-MM-YYYY hh:mm A")
              : "-"}
          </Text>
          {cell?.row?.values.lastCommunicationDate && (
            <Text
    ...
    {
      Header: t("modules.lead.view.columns.scheduledCommunication"),
      accessor: "scheduledCommunicationDate",
      cell: (cell) => (
        <div className="selectOpt">
          <Text color={textColor} fontSize="sm" fontWeight="700">
            {cell?.row?.values.scheduledCommunicationDate
              ? moment(cell?.row?.values.scheduledCommunicationDate).format("DD-MM-YYYY hh:mm A")
              : "-"}
          </Text>
          {cell?.row?.values.scheduledCommunicationDate && (
            <Text
                ...
                <Card mt={3}>
                  <Grid templateColumns="repeat(12, 1fr)">
                    <GridItem colSpan={{ base: 6 }}>
                      ...
                      <Text>
                        {data?.associatedListing?.name ? data.associatedListing.name : " - "}
                      </Text>
                    </GridItem>
                    <GridItem colSpan={{ base: 6 }}>
                      <Text fontSize="sm" fontWeight="bold" color={"blackAlpha.900"}>
