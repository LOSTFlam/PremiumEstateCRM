...
const ChangeImage = () => {
  ...
  const setImageData = async (item) => {
    try {
      setIsLoding(true);
      let response = await putApi(`api/images/isActive/${item?._id}`, {
        isActive: true,
      });
      if (response?.status === 200) {
        handleViewClose();
                  ...
                  >
                    <div className="imageCard">
                      ...
                      <div className="imageContent">
                        <Button size="sm" variant="brand" onClick={() => setImageData(item)}>
                          Set Image
                        </Button>
                        <Button
                          size="sm"
                        ...
                        <Button
                          size="sm"
                          colorScheme="red"
                          disabled={item?.isActive === true ? true : false}
                          onClick={() => {
