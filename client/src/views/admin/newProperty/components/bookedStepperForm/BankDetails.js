...
export const BankDetails = (props) => {
  const { formik, assignToLeadData, assignToContactData, setSelectedRecord, selectedRecord } =
    props;
  const { onOpen: _onOpen, isOpen: _isOpen, onClose: _onClose } = useDisclosure();
  const [bankAllDetails, setBankAllDetails] = useState([]);
  const initialLimit = 5;
  const [visibleCount, _setVisibleCount] = useState(initialLimit);
  const [bankDetailsModel, setBankDetailsModel] = useState(false);

  const { values, handleChange, setFieldValue, handleBlur, errors, touched, isValid, dirty } =
    formik;

  const findLeadName = assignToLeadData?.find((item) => item?._id === values?.lead);

  const findContactName = assignToContactData?.find((item) => item?._id === values?.contact);
  ...
  const handleRadioChange = (selectedId) => {
    const record = bankAllDetails?.data?.find((item) => item?._id === selectedId);
          ...
          bankAllDetails?.data?.slice(0, visibleCount)?.map((item, index) => (
            <GridItem key={index} colSpan={{ base: 2, sm: 3, md: 4, lg: 2 }}>
              <Card style={{ background: "#F9F9F9" }}>
                <RadioGroup onChange={handleRadioChange} value={selectedRecord?._id}>
      ...
      <Grid templateColumns="repeat(12, 1fr)" gap={3}>
        ...
        <GridItem colSpan={{ base: 6 }}>
          ...
          <Input
            ...
            fontWeight="500"
            borderColor={errors?.accountNumber && touched?.accountNumber ? "red.300" : null}
          />
        ...
        <GridItem colSpan={{ base: 12 }}>
          ...
          <Input
            ...
            fontWeight="500"
            borderColor={errors?.swiftCode && touched?.swiftCode ? "red.300" : null}
          />
        ...
        <GridItem colSpan={{ base: 6 }} display={"flex"} justifyContent={"flex-end"}>
          <Button
