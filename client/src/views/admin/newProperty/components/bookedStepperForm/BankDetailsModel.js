...
const BankDetailsModel = (props) => {
  const { onClose, isOpen, bankAllDetails, selectedRecord, setSelectedRecord, setFieldValue } =
    props;
              ...
              <GridItem key={item?._id} colSpan={{ base: 12, md: 6, lg: 4 }}>
                ...
                >
                  <Box>
                    ...
                    <Text style={{ marginLeft: "20px" }}>Bank: {item?.bank}</Text>
                    <Text style={{ marginLeft: "20px" }}>Branch: {item?.branch}</Text>
                  </Box>
                </div>
              </GridItem>
            ))}
