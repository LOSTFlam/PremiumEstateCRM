      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent>
          <ModalHeader>
            ...
            <sup>{data?.floorName?.floorNumberSuffix}</sup> Floor - {data?.unit?.flateName}
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Grid templateColumns="repeat(12, 1fr)" gap={3}>
              <GridItem colSpan={{ base: 6 }}>
                ...
                <Text>{data?.unitType?.name ? data?.unitType?.name : " - "}</Text>
              </GridItem>
              <GridItem colSpan={{ base: 6 }}>
                <Text fontSize="sm" fontWeight="bold" color="blackAlpha.900">
                  Sqm
          ...
          <ModalFooter>
            <Flex>
              <Select placeholder={"Unit Type"} value={selectedUnitType} onChange={handleChange}>
                {unitTypeList?.map((unitType) => {
                  return (
                    <option key={unitType?._id || unitType?.name} value={unitType?._id}>
                      {unitType?.name}
                    </option>
                  );
                })}
