...
import { Flex, useColorModeValue, Select, Button, Grid, GridItem, Text } from "@chakra-ui/react";
...
function PropertyImport() {
  ...
  const _borderColor = useColorModeValue("gray.200", "whiteAlpha.100");
  const [importedFileData, setImportedFileData] = useState([]);
  const [_isLoding, setIsLoding] = useState(false);
  ...
  const fieldsInCrm = [
    ...(customFields || []).map((field) => ({
      ...
      formikType: field?.validations?.find((obj) =>
        Object.prototype.hasOwnProperty.call(obj || {}, "formikType")
      ),
    })),
  ];
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm: _resetForm }) => {
      const leadsData = importedFileData?.map((item, _ind) => {
        const lead = {
  ...
  return (
    <>
      <Card overflowY={"auto"} className="importTable">
        <Text color={"secondaryGray.900"} fontSize="22px" fontWeight="700" mb="20px">
          Import Properties{" "}
        </Text>
        <Grid
              ...
              <GridItem colSpan={{ base: 4 }}>
                ...
                >
                  <option value="">
                    ...
                    {filterProperty
                      ? filterProperty?.find(
                          (data) => (item?.Header === data || item?.accessor === data) && data
                        )
                        ? filterProperty?.find(
                            (data) => (item?.Header === data || item?.accessor === data) && data
                          )
