import { AddIcon, ChevronDownIcon, DeleteIcon, EditIcon, ViewIcon } from "@chakra-ui/icons";
import {
  Box,
  Button,
  Flex,
...
import { MdMoveDown, MdMoveUp, MdBlock } from "react-icons/md";
import { TbStatusChange } from "react-icons/tb";
import { useDispatch, useSelector } from "react-redux";
import { Link, useNavigate, useParams } from "react-router-dom";
import { deleteApi, getApi, postApi, postApiBlob as _postApiBlob } from "services/api";
...
const View = () => {
  ...
  const [permission, emailAccess, callAccess] = HasAccess(["Properties", "Emails", "Calls"]);
  ...
  const handleStatusChange = async (floor, selectedItem, newStatus) => {
    try {
      ...
      setIsLoding(true);
      let response = await postApi(`api/property/change-unit-status/${param?.id}`, paylaod);
      if (response && response?.status === 200) {
        setAction((pre) => !pre);
      }
    } catch (e) {
  ...
  const statusCount = data?.units?.reduce((acc, floor) => {
    floor?.flats?.forEach((flat) => {
      const status = flat?.status?.charAt(0)?.toUpperCase() + flat?.status?.slice(1)?.toLowerCase();
              ...
              <TabPanel pt={4} p={0}>
                ...
                {(data?.units || []).map((floor) =>
                  Array?.isArray(floor?.flats) && floor?.flats?.length > 0 ? (
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    handleViewUnitType(item, {
                                      ...
                                      floorNumberSuffix: getOrdinalSuffix(floor?.floorNumber),
                                    })
                                  }
                                  me={2}
                                  colorScheme="green"
                                ...
                                <Menu isLazy placement="top">
                                  ...
                                  <MenuList minW={"fit-content"}>
                                    {item?.status !== "Available" && (
                                      <MenuItem
                                        py={2.5}
                                        icon={<TbStatusChange fontSize={15} mb={1} />}
                                        onClick={() => {
                                    ...
                                    {!["Booked", "Sold", "Blocked"]?.includes(item?.status) && (
                                      <MenuItem
                                        py={2.5}
                                        icon={<TbStatusChange fontSize={15} mb={1} />}
                                        onClick={() => {
                    ...
                    <Card key={floor?._id || floor?.floorNumber} mt="5">
                      <Text
                        textAlign={"center"}
                        width="100%"
                        color={"gray.500"}
                        ...
                        <GridItem colSpan={12}>
                          <Box>
                            <Flex
                              ...
                              alingItem={"center"}
                            >
                              <Heading size="md">Virtual Tours or Videos</Heading>
                              <Button
                                size="sm"
                                leftIcon={<AddIcon />}
                        ...
                        <GridItem colSpan={12} sx={{ maxHeight: "200px", overflowX: "auto" }}>
                          {data?.propertyDocuments?.length > 0 ? (
                            ...
                            data?.propertyDocuments?.map((item) => {
                              ...
                              return (
                                <Flex
                                  key={item?._id || item?.filename || item?.img}
                                  alignItems={"center"}
                                  mt="3"
                                >
          ...
          <ModalBody overflowY={"auto"} height={"700px"}>
            {data?.propertyDocuments?.length > 0 ? (
              ...
              data?.propertyDocuments?.map((item) => {
                ...
                const lastIndex = parts[parts?.length - 1];
                return (
                  <Flex key={item?._id || item?.filename || item?.img} alignItems={"center"} mt="3">
                    {lastIndex === "xlsx" && <Image src={xlsx} boxSize="50px" />}
                    {lastIndex === "jpg" && <Image src={jpg} boxSize="50px" />}
                    {lastIndex === "png" && <Image src={png} boxSize="50px" />}
                    {lastIndex === "pdf" && <Image src={pdf} boxSize="50px" />}
