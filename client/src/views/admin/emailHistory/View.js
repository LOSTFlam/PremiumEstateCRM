...
import { fetchPropertyData as _fetchPropertyData } from "../../../redux/slices/propertySlice";
                ...
                <Grid gap={4}>
                  ...
                  <GridItem colSpan={{ base: 2, md: 1 }}>
                    ...
                    {data?.createBy ? (
                      <Link to={`/contactView/${data?.createBy}`}>
                        <Text
                          color={contactAccess?.view ? "brand.600" : "blackAlpha.900"}
                          sx={{
                            "&:hover": {
                      ...
                      <Link to={`/leadView/${data?.createByLead}`}>
                        <Text
                          color={leadAccess?.view ? "brand.600" : "blackAlpha.900"}
                          sx={{
                            "&:hover": {
                              color: leadAccess?.view ? "blue.500" : "blackAlpha.900",
                              textDecoration: leadAccess?.view ? "underline" : "none",
                            },
                          }}
                        >
                          {data?.createByName ? data?.createByName : " - "}
                  ...
                  <GridItem colSpan={{ base: 2, md: 1 }}>
                    ...
                    <Text>
                      ...
                      {data?.timestamp ? moment(data?.timestamp).toNow() : " - "}]
                    </Text>
                  </GridItem>
                  <GridItem colSpan={{ base: 2, md: 1 }}>
                    <Text fontSize="sm" fontWeight="bold" color={"blackAlpha.900"}>
          ...
          <Grid templateColumns="repeat(2, 1fr)" gap={1} mt={3}>
            <GridItem colSpan={{ base: 2 }}>
              <CommonCheckTable
                ...
                AdvanceSearch={false}
                tableCustomFields={
                  data?.properties?.[0]?.fields?.filter((field) => field?.isTableField === true) ||
                  []
                }
                addBtn={false}
                ManageGrid={false}
