...
const Task = () => {
  ...
  const [permission] = HasAccess(["Tasks", "Leads", "Contacts"]);
  ...
  const tableColumns = [
    ...
    {
      Header: t?.("fields.related") || "Related",
      accessor: "category",
      type: "text",
      formikType: "",
    },
    ...
    {
      Header: t?.("fields.startDate") || "Start Date",
      accessor: "start",
      type: "date",
      formikType: "",
    },
  ...
  const setStatusData = async (cell, e) => {
    try {
      setIsLoding(true);
      let response = await putApi(`api/task/changeStatus/${cell?.row?.original?._id}`, {
