...
const View = (props) => {
  const { onClose, isOpen, selectedId: _selectedId, fetchData, setAction } = props;
  const [isLoding, _setIsLoding] = useState(false);
  const [data, _setData] = useState(false);
            ...
            <Box>
              <Grid templateColumns="repeat(12, 1fr)" gap={3}>
                <GridItem colSpan={{ base: 12 }}>
                  <Flex>
                    <Text fontWeight={"bold"} pr={2} textTransform={"capitalize"}>
                      require :
                    </Text>
                    <Text>
                      {data?.validations &&
