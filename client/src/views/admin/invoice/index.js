...
const Index = (_props) => {
  ...
  const [_selectedPreviewId, _setSelectedPreviewId] = useState();
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (fetchInvoicesData.rejected.match(result)) {
  ...
  const [columns, _setColumns] = useState([...tableColumns]);
  const [selectedColumns, _setSelectedColumns] = useState([...tableColumns]);
  const _dataColumn = tableColumns?.filter((item) =>
