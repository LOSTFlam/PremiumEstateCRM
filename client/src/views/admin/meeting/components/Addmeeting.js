...
const AddMeeting = (props) => {
  const { onClose, isOpen, setAction: _setAction, from: _from, fetchData, view, leadName } = props;
  ...
  const [_setCondition, setSetCondition] = useState([]);
  ...
  const _findLeadName = leaddata?.find((item) => item?.leadName === leadName?.leadName);
  ...
  useEffect(() => {
    const conditionData = values?.related === "Contact" ? contactdata : leadData;
