...
const PropertyMetric = ({ icon, label, value, iconColor }) => (
  ...
  >
    <HStack spacing={{ base: 3, md: 4 }} align="start" wrap="wrap">
      <Circle size={{ base: "44px", md: "50px" }} bg={iconColor} color="white" flexShrink={0}>
...
const PropertyFactPill = ({ fact, compact = false }) => (
  <HStack
    ...
    align="start"
  >
    <Circle size={compact ? "34px" : "40px"} bg={fact.color} color="white" flexShrink={0}>
    ...
    <Box minW={0}>
      ...
      <Text fontWeight="700" color="gray.800" fontSize={compact ? "md" : "lg"} noOfLines={1}>
        {fact.value}
      </Text>
    </Box>
  </HStack>
...
export default function PropertyLandingPage() {
  ...
  const borderColor = useColorModeValue("rgba(18,55,42,0.08)", "rgba(255,255,255,0.1)");
  ...
  const subtlePanel = useColorModeValue("rgba(255,255,255,0.12)", "rgba(255,255,255,0.08)");
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      ...
      try {
        const endpoint =
          user?.role === "superAdmin" ? "api/property/" : `api/property/?createBy=${user?._id}`;
  ...
  const uniqueStatuses = useMemo(
    () =>
      Array.from(
        new Set(
          properties.map((property) => normalizeStatus(property?.listingStatus, t, i18n.language))
        )
      ).filter(Boolean),
  ...
  const uniqueTypes = useMemo(
    () =>
      Array.from(
        new Set(properties.map((property) => getPropertyType(property, t, i18n.language)))
      ).filter(Boolean),
    [i18n.language, properties, t]
  );
  ...
  const filteredProperties = useMemo(() => {
    ...
    const list = properties.filter((property) => {
      ...
      const matchesType =
        typeFilter === "all" || getPropertyType(property, t, i18n.language) === typeFilter;
  ...
  const availableInventory = useMemo(
    () =>
      properties.filter((property) => {
        ...
        return ["available", "active", "new", "доступно", "активно", "новый", "новое"].includes(
          status
        );
      }).length,
    [properties]
  ...
  const overviewMetrics = useMemo(
    ...
    [filteredProperties.length, sortModeLabel, t, uniqueStatuses.length, uniqueTypes.length]
  );
      ...
      <Box maxW={PAGE_MAX_W} mx="auto">
        <Stack spacing={{ base: 6, xl: 8 }}>
          ...
          >
            ...
            />
            <Stack spacing={{ base: 6, xl: 8 }} p={{ base: 6, md: 8, xl: 10 }} position="relative">
              <Wrap spacing={3}>
                <WrapItem>
          ...
          <Card bg={surfaceBg} border={`1px solid ${borderColor}`} boxShadow={softShadow}>
            <Grid
              templateColumns={{
                base: "1fr",
              ...
              <GridItem>
                <Stack spacing={4}>
                  <HStack spacing={4} align="start">
                    <Circle size="48px" bg="green.50" color="green.700" flexShrink={0}>
                  ...
                  <Wrap spacing={2.5}>
                    {activeFilterChips.length ? (
                      activeFilterChips.map((item) => <FilterChip key={item} label={item} />)
                    ) : (
                      <FilterChip label={t?.("modules.dashboardHome.newestFirst")} />
                    )}
                  </Wrap>
                </Stack>
              </GridItem>
          ...
          {isLoading ? (
              ...
              {featuredProperty ? (
                ...
                >
                  <Grid
                    ...
                    gap={0}
                  >
                    <GridItem position="relative" minH={{ base: "320px", xl: "100%" }}>
                      <Image
                      ...
                      <Wrap position="absolute" top={5} left={5} spacing={3}>
                        <WrapItem>
                          <Badge
                            colorScheme={
                              statusColorMap[getStatusKey(featuredProperty?.listingStatus)] ||
                              "gray"
                            }
                            px={4}
                            py={2}
                    ...
                    <GridItem>
                      <Stack spacing={6} p={{ base: 6, md: 8, xl: 10 }}>
                        ...
                        <SimpleGrid columns={{ base: 1, md: 3 }} gap={3}>
                          {buildPropertyFacts(featuredProperty, t, i18n.language).map((fact) => (
              ...
              {remainingProperties.length > 0 ? (
                <SimpleGrid columns={PROPERTY_GRID_COLUMNS} gap={6}>
                  {remainingProperties.map((property) => {
                    ...
                    const statusColor =
                      statusColorMap[getStatusKey(property?.listingStatus)] || "gray";
                      ...
                      >
                        ...
                        <Stack spacing={5} p={{ base: 5, md: 6 }}>
                          ...
                          <SimpleGrid columns={{ base: 1, sm: 3 }} gap={3}>
                            {buildPropertyFacts(property, t, i18n.language).map((fact) => (
                              <PropertyFactPill
            ...
            <Card bg={surfaceBg} border={`1px solid ${borderColor}`}>
              <Stack spacing={4} align="start">
                ...
                <SimpleGrid columns={{ base: 1, md: 2 }} gap={3} w={{ base: "100%", md: "auto" }}>
