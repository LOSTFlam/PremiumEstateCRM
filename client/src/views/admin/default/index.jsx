...
const PropertyMetric = ({ icon, label, value, iconColor, valueSize = "md" }) => (
  <Box borderRadius="24px" px={{ base: 5, md: 6 }} py={{ base: 4, md: 5 }} bg="rgba(255,255,255,0.72)" border="1px solid rgba(18,55,42,0.08)" backdropFilter="blur(8px)" minW={0} w="100%">
    <HStack spacing={{ base: 3, md: 4 }} align="start" wrap="wrap">
      ...
      <Box flex={1} minW={0}>
        <Text fontSize={{ base: "sm", md: "md" }} textTransform="uppercase" letterSpacing="0.08em" color="gray.500" noOfLines={2} lineHeight="1.3" sx={{ wordWrap: "break-word", overflowWrap: "break-word" }}>
          {label}
        </Text>
        <Text fontWeight="800" fontSize={{ base: "xl", md: "2xl" }} color="gray.800" mt={2} lineHeight="1.2" sx={{ wordWrap: "break-word", overflowWrap: "break-word" }}>
          {value}
        </Text>
      </Box>
    </HStack>
...
export default function PropertyLandingPage() {
  ...
  return (
    <Box minH="100vh" bg={pageBg} px={{ base: 4, md: 6, lg: 8 }} py={{ base: 5, md: 6, lg: 8 }}>
      <Stack spacing={{ base: 6, md: 8, lg: 10 }}>
        {/* Hero Section - Wide Layout */}
        <Card overflow="hidden" border="1px solid rgba(255,255,255,0.18)" bg={heroGradient} color="white" boxShadow={softShadow} position="relative" borderRadius="32px">
          ...
          <Grid templateColumns={{ base: "1fr", lg: "1.2fr 0.8fr" }} gap={{ base: 6, lg: 8 }} position="relative">
            <GridItem>
              <Stack spacing={{ base: 5, md: 6 }} p={{ base: 6, md: 8, lg: 10 }}>
                <Wrap spacing={3}>
                  <WrapItem><Badge px={4} py={1.5} borderRadius="full" bg="whiteAlpha.230" color="white" fontSize="sm" fontWeight="600">{t?.("modules.dashboardHome.badgePrimary")}</Badge></WrapItem>
                  <WrapItem><Badge px={4} py={1.5} borderRadius="full" bg="rgba(255,255,255,0.14)" color="whiteAlpha.900" fontSize="sm" fontWeight="600">{t?.("modules.dashboardHome.badgeSecondary")}</Badge></WrapItem>
                </Wrap>
                <Heading size={{ base: "xl", md: "2xl", lg: "3xl" }} lineHeight="1.1" maxW="900px" fontWeight="800">{t?.("modules.dashboardHome.heroTitle")}</Heading>
                <Text color="whiteAlpha.900" fontSize={{ base: "md", lg: "lg" }} maxW="750px" lineHeight="1.6">{t?.("modules.dashboardHome.heroDescription")}</Text>
                {/* Stats - 3 Equal Columns */}
                <HStack spacing={{ base: 4, md: 5, lg: 6 }} wrap="wrap" w="100%">
                  <Box flex={{ base: "1 1 calc(33.333% - 14px)", md: "1 1 calc(33.333% - 16px)" }} minW="180px">
                    <Stat><StatLabel color="whiteAlpha.800" fontSize={{ base: "sm", md: "md" }} fontWeight="500" mb={2}>{t?.("modules.dashboardHome.totalInventory")}</StatLabel><StatNumber fontSize={{ base: "3xl", md: "4xl", lg: "5xl" }} fontWeight="800">{properties.length}</StatNumber></Stat>
                  </Box>
                  <Box flex={{ base: "1 1 calc(33.333% - 14px)", md: "1 1 calc(33.333% - 16px)" }} minW="180px">
                    <Stat><StatLabel color="whiteAlpha.800" fontSize={{ base: "sm", md: "md" }} fontWeight="500" mb={2}>{t?.("modules.dashboardHome.openInventory")}</StatLabel><StatNumber fontSize={{ base: "3xl", md: "4xl", lg: "5xl" }} fontWeight="800">{availableInventory}</StatNumber></Stat>
                  </Box>
                  <Box flex={{ base: "1 1 calc(33.333% - 14px)", md: "1 1 calc(33.333% - 16px)" }} minW="180px">
                    <Stat><StatLabel color="whiteAlpha.800" fontSize={{ base: "sm", md: "md" }} fontWeight="500" mb={2}>{t?.("modules.dashboardHome.averagePrice")}</StatLabel><StatNumber fontSize={{ base: "2xl", md: "3xl", lg: "4xl" }} fontWeight="700">{averagePrice}</StatNumber></Stat>
                  </Box>
                </HStack>
              </Stack>
            </GridItem>
            <GridItem>
              <Card bg="rgba(255,255,255,0.94)" color="gray.900" borderRadius="28px" boxShadow="0 20px 60px rgba(0,0,0,0.16)" p={{ base: 6, md: 8 }}>
                <Stack spacing={{ base: 4, md: 5 }}>
                  <Flex justify="space-between" align="center">
                    <Heading size="md">{t?.("modules.dashboardHome.findInventory")}</Heading>
                    <Circle size="42px" bg="green.50" color="green.700"><Icon as={MdOutlineTune} boxSize={5} /></Circle>
                  </Flex>
                  <InputGroup>
                    <InputLeftElement pointerEvents="none"><Icon as={LuSearch} color="gray.400" /></InputLeftElement>
                    <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t?.("modules.dashboardHome.searchPlaceholder")} bg="white" borderRadius="18px" h="52px" />
                  </InputGroup>
                  ...
                  <HStack spacing={3} w="100%">
                    <Button flex={1} colorScheme="green" borderRadius="18px" onClick={() => navigate("/properties")} h="50px" fontSize="md">{t?.("modules.dashboardHome.fullTable")}</Button>
                    <Button flex={1} variant="outline" borderRadius="18px" onClick={resetFilters} h="50px" fontSize="md">{t?.("common.reset")}</Button>
                  </HStack>
                </Stack>
              </Card>
            </GridItem>
        ...
        {/* Metrics Row */}
        <SimpleGrid columns={{ base: 1, md: 2, xl: 3 }} gap={{ base: 5, md: 6, lg: 8 }}>
          <PropertyMetric icon={LuBuilding2} label={t?.("modules.dashboardHome.filteredResults")} value={filteredProperties.length} iconColor="green.500" valueSize="lg" />
          <PropertyMetric icon={MdOutlineLocationOn} label={t?.("modules.dashboardHome.propertyTypes")} value={uniqueTypes.length || 0} iconColor="teal.500" valueSize="lg" />
          <PropertyMetric icon={LuSparkles} label={t?.("modules.dashboardHome.statusesUsed")} value={uniqueStatuses.length || 0} iconColor="orange.400" valueSize="lg" />
        </SimpleGrid>

        {/* Featured Property */}
        {featuredProperty && (
          <Card p={0} overflow="hidden" bg={surfaceBg} border={`1px solid ${borderColor}`} backdropFilter="blur(10px)" borderRadius="30px">
            <Grid templateColumns={{ base: "1fr", xl: "1fr 1fr" }}>
              <GridItem position="relative" minH={{ base: "280px", xl: "100%" }}>
                ...
                <Box position="absolute" inset="0" bg="linear-gradient(180deg, rgba(0,0,0,0.05), rgba(0,0,0,0.56))" />
              ...
              <GridItem>
                <Stack spacing={{ base: 5, md: 6 }} p={{ base: 6, md: 8, lg: 10 }}>
                  ...
                  <HStack spacing={{ base: 3, md: 4 }} wrap="wrap" w="100%">
                    <HStack flex={{ base: "1 1 28%", md: "1 1 auto" }} spacing={3} p={3} borderRadius="16px" bg="rgba(255,255,255,0.6)" border="1px solid rgba(0,0,0,0.05)">
                    ...
                    <HStack flex={{ base: "1 1 28%", md: "1 1 auto" }} spacing={4} p={3} borderRadius="16px" bg="rgba(255,255,255,0.6)" border="1px solid rgba(0,0,0,0.08)" boxShadow="0 4px 12px rgba(0,0,0,0.08)">
                  ...
                  <Button colorScheme="green" borderRadius="18px" onClick={() => navigate(`/propertyView/${featuredProperty?._id}`)} h="50px" fontSize="md" rightIcon={<MdArrowForward />}>{t?.("modules.dashboardHome.viewProperty")}</Button>
                </Stack>
              </GridItem>
            </Grid>
          </Card>
        ...
        {remainingProperties.length > 0 && (
          <SimpleGrid columns={{ base: 1, md: 2, xl: 3 }} gap={{ base: 5, md: 6, lg: 8 }}>
            {remainingProperties.map((property) => {
              ...
              return (
                <Card key={property?._id} p={0} overflow="hidden" bg={surfaceBg} border={`1px solid ${borderColor}`} backdropFilter="blur(10px)" borderRadius="26px" transition="transform 0.2s ease, box-shadow 0.2s ease" _hover={{ transform: "translateY(-4px)", boxShadow: "0 20px 60px rgba(15,47,36,0.12)" }}>
                  ...
                  <Stack spacing={{ base: 4, md: 5 }} p={{ base: 5, md: 6 }}>
                    ...
                    <Button size="sm" borderRadius="18px" colorScheme="green" rightIcon={<MdArrowForward />} onClick={() => navigate(`/propertyView/${property?._id}`)} h="46px" fontSize="md">{t?.("modules.dashboardHome.openProperty")}</Button>
                  </Stack>
                </Card>
              );
            })}
