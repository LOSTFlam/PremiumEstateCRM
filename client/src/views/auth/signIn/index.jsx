...
function SignIn() {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (result?.response) {
        const errorMsg = result.response?.data?.error || result.response?.data?.message || "Login failed";
      ...
      if (result && (result.status === 200 || result.status === 201) && result.data && result.data.user) {
        ...
        setTimeout(() => {
          ...
          console.log("Is user role 'user'?", currentUser?.role === "user");
          console.log("Is user role 'superAdmin'?", currentUser?.role === "superAdmin");

          if (currentUser?.role === "user" || currentUser?.role === "superAdmin") {
            console.log("✅ Navigating to dashboard with role:", currentUser?.role);
            // Force reload to ensure App component re-renders with new auth state
            window.location.href = "/dashboard";
          } else {
            console.log("⚠️ Unknown role:", currentUser?.role, "staying on sign-in");
