...
export const useProperties = (filters: PropertyFilters = {}) => {
  return useQuery<IProperty[]>({
    ...
    queryFn: async () => {
      const response = await getApi('api/property/public') as { data?: IProperty[] } | IProperty[];
...
export const useProperty = (id: string | undefined) => {
  return useQuery<IProperty | null>({
    ...
    queryFn: async () => {
      const response = await getApi(`api/property/public/${id}`) as { data?: IProperty } | IProperty;
