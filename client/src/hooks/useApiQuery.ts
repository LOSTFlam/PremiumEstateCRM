...
interface UseApiQueryOptions<TData = unknown> extends Omit<UseQueryOptions<TData, Error, TData>, 'queryKey' | 'queryFn'> {
  params?: Record<string, unknown>;
  useCache?: boolean;
}

interface UseApiMutationOptions<TData = unknown, TVariables = unknown> extends Omit<UseMutationOptions<TData, Error, TVariables>, 'mutationFn'> {
...
export const useApiPaginatedQuery = <TData = unknown>(
  ...
  return useQuery<PaginatedResponse<TData>, Error>({
    ...
    queryFn: async () => {
      ...
      return (response?.data || { data: [], pagination: { page, limit, total: 0, totalPages: 0 }, success: true }) as PaginatedResponse<TData>;
    },
