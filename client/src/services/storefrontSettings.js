...
const normalizeResponsePresets = (response) => mergeStorefrontPresets(response?.presets || response?.data?.presets || []);
...
export const fetchPublicStorefrontSettings = async () => {
  try {
    ...
    console.error("Failed to fetch public storefront settings:", error);
    return {
