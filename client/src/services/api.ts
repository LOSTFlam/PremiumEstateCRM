...
const buildHeaders = (isFormData = false, headers: Record<string, string> = {}): Record<string, string> => {
...
const normalizeWriteArgs = (loginOrOptions?: PersistOptions | boolean, isFormData?: boolean): PersistOptions => {
...
export const postApi = async (
  ...
  const { rememberMe, isFormData: useFormData, requestConfig } = normalizeWriteArgs(login, isFormData);
...
export const putApi = async (path: string, data: unknown, _id?: string | number): Promise<AxiosResponse> => {
...
export const getApi = async (path: string, id?: string | number | Record<string, unknown>): Promise<unknown> => {
