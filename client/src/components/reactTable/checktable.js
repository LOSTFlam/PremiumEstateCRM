...
import _CountUpComponent from "components/countUpComponent/countUpComponent";
...
const CommonCheckTable = (props) => {
          ...
          return tableCustomFields?.every((field) => {
            ...
            if (field?.type === "select") {
              ...
              return (
                ...
                (itemValue !== undefined && itemValue.toString()?.includes(fieldValue?.toString()))
              );
              ...
              return (
                (!fromDate || timeMomentDate >= fromDate) && (!toDate || timeMomentDate <= toDate)
              );
            } else {
  ...
  const toggleColumnVisibility = (columnKey) => {
    ...
    if (tempSelectedColumns?.some((column, index) => getColumnKey(column, index) === columnKey)) {
    ...
    const orderedColumns = columnData?.filter((column) =>
      updatedColumns.some(
        (updatedColumn, index) => getColumnKey(updatedColumn, index) === getColumnKey(column, index)
      )
    );
  ...
  const handleRemoveFromTag = (name) => {
    ...
    for (let key in updatedSearchValue) {
      if (Object.prototype.hasOwnProperty.call(updatedSearchValue, key)) {
        ...
        <Grid templateColumns="repeat(12, 1fr)" gap={2}>
          <GridItem colSpan={{ base: 12, md: 8 }} display={"flex"} alignItems={"center"}>
            <Flex alignItems={"center"} flexWrap={"wrap"}>
              ...
              {customSearch !== false && (
                <CustomSearchInput
                  setSearchbox={setSearchboxOutside ? setSearchboxOutside : setSearchbox}
                  setDisplaySearchData={
                    setSearchboxOutside ? props?.setSearchDisplay : setDisplaySearchData
                  }
                  ...
                  setGetTagValues={
                    props?.setGetTagValuesOutside ? props?.setGetTagValuesOutside : setGetTagValues
                  }
              ...
              <Menu isLazy>
                ...
                <MenuList minW={"fit-content"} transform={"translate(1670px, 60px)"} zIndex={2}>
                  ...
                  {exportColumn !== false && allData && allData?.length > 0 && (
                    <>
                      <MenuDivider />
                      <MenuItem width={"165px"} onClick={() => handleExportLeads("csv")}>
                        {selectedValues && selectedValues?.length > 0
                          ? isRu
                            ? "Экспорт выбранного в таблицу CSV"
                            : "Export Selected Data as CSV"
                          : isRu
                            ? "Экспорт в таблицу CSV"
                            : "Export as CSV"}
                      </MenuItem>
                      <MenuItem width={"165px"} onClick={() => handleExportLeads("xlsx")}>
                        {selectedValues && selectedValues?.length > 0
                          ? isRu
                            ? "Экспорт выбранного в таблицу Excel"
                  ...
                  >
                    {headerGroup?.headers?.map((column, index) => {
                      const headerProps = column?.getHeaderProps(
                        column?.isSortable !== false && column?.getSortByToggleProps()
                      );
                      return (
                        <Th
                          {...headerProps}
            ...
            <Tbody {...getTableBodyProps()}>
              {isLoding ? (
                ...
                page?.map((row, i) => {
                  ...
                  return (
                    <Tr {...rowProps} key={row?.id || rowProps?.key || `row-${i}`}>
                      {row?.cells?.map((cell, index) => {
                        ...
                        columnData?.forEach((item) => {
                          if (cell?.column?.Header === item?.Header) {
                            if (item?.cell && typeof item?.cell === "function") {
                              ...
                              data = (
                                <Flex align="center">
                                  {item?.Header === "#" && (checkBox || checkBox === undefined) && (
                                    <Checkbox
                        ...
                        return (
                          <Td
                            {...cellProps}
                            key={cell?.column?.id || cellProps?.key || `${row?.id || i}-${index}`}
                            fontSize={{ sm: "14px" }}
                    ...
                    <Text display={"flex"} key={columnKey} py={2}>
                      <Checkbox
                        defaultChecked={columns?.some(
                          (item, itemIndex) => getColumnKey(item, itemIndex) === columnKey
                        )}
