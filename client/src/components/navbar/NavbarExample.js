...
export default function AuthNavbar(props) {
  const { logo: _logo, logoText, secondary: _secondary, sidebarWidth, ...rest } = props;
  ...
  const createPagesLinks = (routes) => {
    return routes?.map((link) => {
      ...
      if (link?.name === "Pricing Page") {
        return (
          <Stack key={link?.path || link?.name} direction="column">
      ...
      if (link?.authIcon) {
        ...
        if (link.component) {
          return (
            <NavLink key={link?.path || link?.name} to={link?.layout + link?.path}>
              <MenuItem
                ps="36px"
                py="0px"
          ...
          return (
            <React.Fragment key={link?.path || link?.name}>
              {createPagesLinks(link.items)}
            </React.Fragment>
          );
        }
  ...
  const createAuthLinks = (routes) => {
    return routes?.map((link) => {
      if (link?.authIcon && link?.collapse === true) {
        return (
          <Stack key={link?.path || link?.name} direction="column" my="auto">
  ...
  const linksAuth = (
    <HStack display={{ sm: "none", lg: "flex" }} spacing="12px">
      ...
      >
        ...
        <Icon as={GoChevronDown} color={mainText} w="14px" h="14px" fontWeight="2000" />
          ...
          >
            <Stack direction="row" spacing="24px">
              <Flex
                ...
                borderRadius="15px"
              >
                <IconBox bg="white" color="white" borderRadius="50%" h="50px" w="50px" mb="12px">
                  <Icon as={AiFillStar} w="25px" h="25px" color="blue.500" />
                </IconBox>
                <Text fontSize="xl" fontWeight="bold" color="#fff" maxW="80%" textAlign="center">
