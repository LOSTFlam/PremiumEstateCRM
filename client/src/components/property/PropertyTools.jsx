...
export const InvestmentCalculator = ({ propertyPrice, isOpen, onClose }) => {
  ...
  const calculations = {
    monthlyMortgage: ((purchasePrice - downPayment) * (interestRate / 100 / 12)) /
      ...
      <ModalContent bg={bgColor} borderRadius="2xl">
        <ModalHeader>
          <Flex align="center" gap={3}>
            ...
            <Text fontSize="2xl" fontWeight="bold">Investment Calculator</Text>
          </Flex>
        </ModalHeader>
        <ModalCloseButton />
        ...
        <ModalBody>
          <VStack spacing={6} align="stretch">
            <Flex gap={4}>
              <Box flex={1}>
                <Heading size="md" mb={4}>Investment Details</Heading>
                <FormControl mb={4}>
            ...
            <Box bg="green.50" p={6} borderRadius="xl">
              <Heading size="lg" mb={4} color="green.700">Returns</Heading>
              <Flex gap={4}>
                <Box flex={1} bg="white" p={4} borderRadius="lg" textAlign="center">
                  <Text fontSize="sm" color="gray.600">Monthly Cash Flow</Text>
                  <Text fontSize="3xl" fontWeight="bold" color={monthlyCashFlow >= 0 ? 'green.500' : 'red.500'}>
                    ${monthlyCashFlow.toFixed(2)}
                  </Text>
                </Box>
                <Box flex={1} bg="white" p={4} borderRadius="lg" textAlign="center">
                  <Text fontSize="sm" color="gray.600">Cash on Cash Return</Text>
                  <Text fontSize="3xl" fontWeight="bold" color={parseFloat(cashOnCashReturn) >= 8 ? 'green.500' : 'orange.500'}>
                    {cashOnCashReturn}%
                  </Text>
                </Box>
                <Box flex={1} bg="white" p={4} borderRadius="lg" textAlign="center">
                  <Text fontSize="sm" color="gray.600">Cap Rate</Text>
                  <Text fontSize="3xl" fontWeight="bold" color={parseFloat(capRate) >= 6 ? 'green.500' : 'orange.500'}>
                    {capRate}%
                  </Text>
                </Box>
              </Flex>
            ...
            <Box bg="blue.50" p={4} borderRadius="lg">
              <Text fontSize="sm" fontWeight="600">
                💡 Tip: A good investment property should have a cash on cash return of at least 8-12%
              </Text>
            </Box>
          </VStack>
        </ModalBody>
...
export const SavedSearches = () => {
  ...
  return (
    <Box>
      <Heading size="md" mb={4}>Saved Searches</Heading>
          ...
          >
            <Flex justify="space-between" align="center">
              <Box>
                ...
                <Text fontSize="sm" color="gray.600">{search.criteria}</Text>
              </Box>
              <Flex gap={2}>
                <Icon
                  as={FiBell}
...
export const PropertyAlerts = () => {
  ...
  return (
    <Box>
      <Heading size="md" mb={4}>Property Alerts</Heading>
      <VStack spacing={3} align="stretch">
        {alerts.map(alert => (
          <Box
            key={alert.id}
      ...
      <ModalContent bg={bgColor} borderRadius="2xl">
        <ModalHeader>
          <Flex align="center" gap={3}>
            ...
            <Text fontSize="2xl" fontWeight="bold">Contact Agent</Text>
          </Flex>
        </ModalHeader>
        <ModalCloseButton />
