...
const SmartField = React.memo(function SmartField({
  field,
  form,
  ...rest
}) {
  const { name, label, type = 'text', placeholder, options = [], icon, min = 0, max = 100, required } = field;
  ...
  return (
    <FormControl isInvalid={showError} isRequired={required}>
      ...
      {type === 'select' ? (
        ...
        <RadioGroup value={String(value ?? '')} onChange={(nextValue) => form.setFieldValue(name, nextValue)}>
        ...
        <InputGroup>
          ...
          <Input
            {...fieldProps}
            type={fieldToInputType[type] || 'text'}
            {...rest}
          />
        </InputGroup>
      )}
