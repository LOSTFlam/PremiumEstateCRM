...
export default function DeepParallaxBackground() {
  ...
  const backgroundLayers = useMemo(() => [
    ...
    },
    {
      ...
      opacity: 0.4,
      background: "radial-gradient(circle at 80% 70%, rgba(255,255,255,0.02) 0%, transparent 45%)",
      blur: "0px",
      zIndex: 0,
      speed: 0.05,
    },
    {
      ...
      opacity: 0.5,
      background: "radial-gradient(circle at 40% 60%, rgba(245,208,118,0.04) 0%, transparent 40%)",
      blur: "20px",
      zIndex: 1,
      speed: 0.08,
    },
    {
      ...
      opacity: 0.6,
      background: "radial-gradient(circle at 60% 40%, rgba(255,255,255,0.03) 0%, transparent 35%)",
      blur: "30px",
      zIndex: 2,
      speed: 0.12,
    },
    ...
    <>
      ...
      {backgroundLayers.map((layer, index) => (
        <Box
          ...
          animation={!prefersReducedMotion ? `float-depth-${(index % 3) + 1} ${25 + index * 5}s ease-in-out infinite` : "none"}
          pointerEvents="none"
        />
      ))}

      {/* Floating particles at different depths */}
      {!prefersReducedMotion && particleLayers.map((particle) => (
        <Box
          key={particle.id}
          position="absolute"
          left={particle.left}
