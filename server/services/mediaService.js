...
const cleanupPropertyFiles = async (property) => {
  ...
  if (photosResult.status === 'rejected') {
    console.error('Failed to delete property photos:', photosResult.reason);
  }
  if (toursResult.status === 'rejected') {
    console.error('Failed to delete virtual tours:', toursResult.reason);
  }
  if (plansResult.status === 'rejected') {
    console.error('Failed to delete floor plans:', plansResult.reason);
  }
  if (documentsResult.status === 'rejected') {
    console.error('Failed to delete property documents:', documentsResult.reason);
  }
...
const collectReferencedFiles = async () => {
  ...
  try {
    ...
    console.error('Failed to collect referenced files from database:', error);
  }
...
const cleanupOrphanedFiles = async (options = {}) => {
  ...
  console.log(`Found ${referencedFilenames.size} referenced files in database`);
...
const optimizeImage = async (inputPath, outputPath = null, options = {}) => {
  ...
  try {
    ...
    console.log(`Optimized image: ${formatFileSize(originalSize)} -> ${formatFileSize(newSize)} (${compressionRatio}% reduction)`);
