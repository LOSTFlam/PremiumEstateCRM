...
const mapPhotos = (propertyType, seed = "", count = 4, date = new Date()) =>
  buildPhotoSet(propertyType, seed, count).map((photo) => ({
    ...photo,
    createOn: date,
  }));
      ...
      await propertiesCollection.updateOne(
        { publicSlug, deleted: false },
        {
          $set: {
            ...
            propertyPhotoSources: photos.map((photo) => photo.img),
            propertyPhotoBackup: photos.map((photo, idx) => ({
              sourceUrl: photo.img,
              ordinal: idx + 1,
              capturedAt: now,
              checksumHint: slugify(`${publicSlug}-${idx}`).slice(0, 32),
            })),
